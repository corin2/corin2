...
<body>
    <form id="form"method="post" action="./login">
        ...
        <div class="col-sm-12">
	        ...
	        <div class="col-md-6">
	        	<button type="button" data-toggle="modal" data-target="#signupmodal" class="btn btn-default">Sign Up</button>
	        </div>
        </div>
        <div class="col-sm-12">
	        <div class="col-md-12">
    ...
    $(function(){
     ...
     <div class="modal fade" id="signupmodal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
	        <form action="signup" id="validation" method="post">
	          <div class="modal-header">
	            <button class="close" type="button" data-dismiss="modal" aria-label="Close">
	              <span aria-hidden="true">×</span>
	            </button>
	          ...
	          <div class="modal-body">
	          	<div>
					<input type="email" class="form-control id" name="userId"
						placeholder="Email" oninput="checkId()" id="modalid" autofocus>
				</div>
				<div>
					<input type="password" class="form-control pass" name="password" id="modalpass"
						placeholder="Password" oninput="checkPwd()">
				</div>
				<div>
					<input type="password" class="form-control pass" name="password2"
						placeholder="Confirm Password" id="modalpass2" oninput="checkPwd()">
				</div>
				<div>
					<input type="text" class="form-control nickname" name="userName"
						id="modalnick" placeholder="Your Nickname" oninput="checkNick()"
						autofocus>
				</div>
	          </div>
	          <div class="modal-footer">
    ...
    <script type="text/javascript">
			...
			function checkId() {
			    var inputed = $('#modalid').val();
			    
			    console.log(inputed);
			    $.ajax({
			    	type: "post",
		 			url:  "idcheck",
		 			data: $('#modalid').val().trim(),
		 			contentType: "application/json; charset=utf-8",
		 		    success:function(data){ 
			        	console.log(data);
			            if(inputed=="" && data=='true'){
			                ...
			                $("#modalid").css("background-color", "#FFCECE");
			                idCheck = 0;
			            } else if (data == 'false') {
			                $("#modalid").css("background-color", "#B0F6AC");
			                idCheck = 1;
			...
			function checkPwd() {
				var inputed = $('#modalpass').val();
				var reinputed = $('#modalpass2').val();
				console.log(inputed);
				console.log(reinputed);
				$.ajax({
					type : "post",
					url : "passwordcheck",
					data : $('#modalpass').val().trim(),
					contentType : "application/json; charset=utf-8",
					success : function(data) {
						if (inputed == "" 
							...
							$(".signupbtn").prop("disabled", true);
							$(".signupbtn").css("background-color", "#aaaaaa");
							$("#modalpass").css("background-color", "#FFCECE");
							
						} else if (inputed == reinputed && data=='false') {
							$("#modalpass").css("background-color", "#B0F6AC");
							$("#modalpass2").css("background-color", "#B0F6AC");
							pwdCheck = 1;
							if (idCheck == 1 && pwdCheck == 1 && nickCheck == 1) {
								$(".signupbtn").prop("disabled", false);
								$(".signupbtn").css("background-color", "#4CAF50");
							...
							$(".signupbtn").css("background-color", "#aaaaaa");
							$("#modalpass").css("background-color", "#FFCECE");
							$("#modalpass2").css("background-color", "#FFCECE");
						}
					}
				});
				$.ajax({
			...
			function checkNick() {
				...
				$.ajax({
					type : "post",
					url : "nickcheck",
					data : $('#modalnick').val().trim(),
					contentType : "application/json; charset=utf-8",
					success : function(data) {
		                if(nickname=="" && data=='true') {
		                    $(".signupbtn").prop("disabled", true);
		                    $(".signupbtn").css("background-color", "#aaaaaa");
		                    $("#modalnick").css("background-color", "#FFCECE");
		                    nickCheck = 0;
		                } else if (data == 'false') {
		                    $("#modalnick").css("background-color", "#B0F6AC");
		                    nickCheck = 1;
