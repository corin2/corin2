/**
    파일명: tagsinput.js
    설   명: 태그생성을 해주는 input-box 컨트롤 해주는 js파일
    사이트:  https://bootstrap-tagsinput.github.io/bootstrap-tagsinput/examples/  
    작성일: 2018. 6. 19.
    작성자: 배현준
*/
	...
	TagsInput.prototype = {
		...
		add : function(item, dontPushVal, options) {
			...
			var itemValue = self.options.itemValue(item), itemText = self.options
					.itemText(item), tagClass = self.options.tagClass(item), itemTitle = self.options
					.itemTitle(item);
			...
			// add a tag element
			var $tag = $('<span class="tag ' + htmlEncode(tagClass)
			...
			if(self.findInputWrapper().closest('div').children('span').length < 5){
				...
				swal({type: 'error',title:'태그는 5개까지 생성 가능합니다.'});
			}
			...
			if (self.isSelect
					&& !$('option[value="' + encodeURIComponent(itemValue)
							+ '"]', self.$element)[0]) {
				var $option = $('<option selected>' + htmlEncode(itemText)
						+ '</option>');
			...
			if (self.objectItems) {
				if (typeof item === "object")
					item = $.grep(self.itemsArray, function(other) {
						return self.options.itemValue(other) == self.options
								.itemValue(item);
					});
				else
					item = $.grep(self.itemsArray, function(other) {
		...
		refresh : function() {
			...
			$('.tag', self.$container)
					.each(
							function() {
								var $tag = $(this), item = $tag.data('item'), itemValue = self.options
								...
								if (self.isSelect) {
									var option = $('option', self.$element)
											.filter(
													function() {
														return $(this).data(
																'item') === item;
		...
		build : function(options) {
			...
			if (self.options.typeaheadjs) {
				...
				self.$input.typeahead(typeaheadConfig, typeaheadDatasets).on(
						'typeahead:selected', $.proxy(function(obj, datum) {
			...
			self.$container.on('keydown', 'input', $.proxy(function(event) {
				...
				var textLength = $input.val().length, wordSpace = Math
						.ceil(textLength / 5), size = textLength + wordSpace
						+ 1;
				$input.attr('size', Math.max(this.inputSize,
						$input.val().length));
			}, self));
									...
									.proxy(
											function(event) {
												...
												var textLength = $input.val().length, wordSpace = Math
														...
														+ wordSpace + 1;
												$input.attr('size', Math.max(
														this.inputSize, $input
																.val().length));
											}, self));

			// Remove icon clicked
			self.$container.on('click', '[data-role=remove]', $.proxy(function(
					event) {
				if (self.$element.attr('disabled')) {
					return;
				}
...
function hashTagText() {
	var hashTag = '';
	$.each($('div[class=bootstrap-tagsinput]').children('span'), function(index, elt) {
		hashTag += elt.innerText;
